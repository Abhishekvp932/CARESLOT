import { IMessage } from '../../models/interface/IMessage';

import { ChatDTO } from '../../types/UserChatDto';

export interface IChatService {
    getUserChat(patientId:string):Promise<ChatDTO[]>;
    sendMessage(chatId:string,content:string,sender:string,type:string,image:string):Promise<IMessage | null>;
    getDoctorChat(doctorId:string):Promise<ChatDTO[]>;
    getDoctorMessage(chatId:string):Promise<IMessage[]>;
    getPatientMessage(chatId:string):Promise<IMessage[]>;
    deleteMessage(messageId:string):Promise<{msg:string}>;
}